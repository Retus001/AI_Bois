...
////// Emissive:
                ...
                float3 Emission = ((floor(_EmissionTex_var.rgb * node_3499) / (node_3499 - 1)*_EmissionColor.rgb*_EmissionPower)+(Diffuse*_ShadowColor.rgb));
                float3 emissive = Emission;
                float node_9996 = posterizeSteps;
                float node_976 = floor((_SpecularPower*pow(max(0,dot(halfDirection,normalDirection)),exp2(_SpecularSize))) * node_9996) / (node_9996 - 1);
                float node_4056 = max(0,dot(normalDirection,lightDirection));
                float node_3226 = posterizeSteps;
                float node_2036 = floor(node_4056 * node_3226) / (node_3226 - 1);
                float node_3373 = posterizeSteps;
                float3 node_2099 = (_LightColor0.rgb*floor(attenuation * node_3373) / (node_3373 - 1)*_LightColor.rgb);
                float3 finalColor = emissive + ((node_976+(node_2036*node_7026))*node_2099);
...
////// Lighting:
                ...
                float node_3373 = posterizeSteps;
                float3 node_2099 = (_LightColor0.rgb*floor(attenuation * node_3373) / (node_3373 - 1)*_LightColor.rgb);
                float3 finalColor = ((node_976+(node_2036*node_7026))*node_2099);
